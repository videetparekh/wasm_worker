...
class ByteStreamReader {
    ...
    readByteArray() {
        ...
        support_1.assert(this.offset + len <= this.bytes.byteLength);
...
class RPCServer {
    ...
    onInitServer(args, header, body) {
        ...
        const asyncInitServer = () => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const gpuDevice = yield webgpu_1.detectGPUDevice();
            ...
            const messageHandler = fcreate((cbytes) => {
                support_1.assert(this.inst !== undefined);
    ...
    checkLittleEndian() {
        ...
        b[2] = 0x33;
        b[3] = 0x44;
        support_1.assert(c[0] === 0x44332211, "RPCServer little endian to work");
    }
    requestBytes(nbytes) {
    ...
    readFromBuffer(nbytes) {
        ...
        let ptr = 0;
        while (ptr < nbytes) {
            support_1.assert(this.messageQueue.length != 0);
